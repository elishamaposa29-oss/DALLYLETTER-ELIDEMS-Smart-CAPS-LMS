...
interface ExerciseQuestionDraft { id:string; prompt:string; type:"input"|"poll"|"drawbox"; marks:number; options:PollOption[]; width:number; height:number; x:number; y:number }
...
export function ExerciseBuilderPanel({lessonId,onSaved}:ExerciseBuilderPanelProps){
 ...
 const addQuestion=(type:ExerciseQuestionDraft["type"]="input")=>setQuestions(c=>[...c,{id:crypto.randomUUID(),prompt:"",type,marks:1,width:80,height:180,x:0,y:0,options:type==="poll"?[{id:crypto.randomUUID(),label:"",value:"A",isCorrect:false},{id:crypto.randomUUID(),label:"",value:"B",isCorrect:false}]:[]}]);
 ...
 return <Card className="border-primary/20 bg-card/80 shadow-sm">
  ...
  <CardContent className="space-y-4">
   ...
   <div className="rounded-xl border bg-background/60 p-3 sm:p-4"><div className="mb-3 flex flex-wrap gap-2"><Button type="button" size="sm" variant="outline" onClick={()=>addQuestion("input")}><Plus className="mr-1 h-4 w-4"/>Question</Button><Button type="button" size="sm" variant="outline" onClick={()=>addQuestion("poll")}><Plus className="mr-1 h-4 w-4"/>Poll</Button><Button type="button" size="sm" variant="outline" onClick={()=>addQuestion("drawbox")}><Plus className="mr-1 h-4 w-4"/>Drawbox</Button></div>
    <div className="relative mx-auto min-h-[720px] max-w-3xl overflow-auto rounded-2xl border-2 border-dashed bg-[#fffdf7] p-4 shadow-inner sm:p-6" onDragOver={e=>e.preventDefault()} onDrop={e=>{const from=Number(e.dataTransfer.getData("text/exercise-index"));if(!Number.isInteger(from)||!questions[from])return;const r=e.currentTarget.getBoundingClientRect();updatePosition(questions[from].id,((e.clientX-r.left)/r.width)*100-questions[from].width/2,e.clientY-r.top)}}><div className="mb-3 flex items-center justify-between"><span className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Book-page workspace</span><span className="text-xs text-muted-foreground">Drag blocks anywhere • adjust W/H/X/Y</span></div><div className="relative min-h-[620px]">{questions.map((q,i)=><div key={q.id} draggable onDragStart={e=>e.dataTransfer.setData("text/exercise-index",String(i))} onDragOver={e=>e.preventDefault()} onDrop={e=>{const from=Number(e.dataTransfer.getData("text/exercise-index"));if(Number.isInteger(from))moveQuestion(from,i)}} style={{position:"absolute",left:`${q.x}%`,top:q.y,width:`${q.width}%`,minHeight:q.height}} className="rounded-lg border bg-card p-3 transition-shadow hover:shadow-md"><div className="mb-2 flex items-center gap-2"><GripVertical className="h-4 w-4 cursor-grab text-muted-foreground"/><Badge variant="secondary">Q{i+1} · {q.type}</Badge><div className="ml-auto flex flex-wrap items-center justify-end gap-1"><Input type="number" min={10} max={100} step={5} className="w-16" value={q.width} onChange={e=>updateQuestion(q.id,{width:Math.max(10,Math.min(100,Number(e.target.value)||10))})} aria-label={`Width for question ${i+1}`}/><Input type="number" min={100} max={1200} step={20} className="w-16" value={q.height} onChange={e=>updateQuestion(q.id,{height:Math.max(100,Math.min(1200,Number(e.target.value)||100))})} aria-label={`Height for question ${i+1}`}/><Input type="number" min={0} max={90} step={1} className="w-16" value={q.x} onChange={e=>updateQuestion(q.id,{x:Math.max(0,Math.min(90,Number(e.target.value)||0))})} aria-label={`X position for question ${i+1}`}/><Input type="number" min={0} max={3000} step={10} className="w-16" value={q.y} onChange={e=>updateQuestion(q.id,{y:Math.max(0,Math.min(3000,Number(e.target.value)||0))})} aria-label={`Y position for question ${i+1}`}/><Input type="number" min={0.5} step="0.5" className="w-20" value={q.marks} onChange={e=>updateQuestion(q.id,{marks:Number(e.target.value)||0})} aria-label={`Marks for question ${i+1}`}/><Button type="button" variant="ghost" size="icon" onClick={()=>removeQuestion(q.id)}><Trash2 className="h-4 w-4 text-destructive"/></Button></div></div><Textarea value={q.prompt} onChange={e=>{updateQuestion(q.id,{prompt:e.target.value});setAiReadiness(null);setAiApproved(false)}} placeholder="Type the exercise question..." className="min-h-20"/>{q.type==="poll"&&<div className="mt-3 space-y-2"><p className="text-xs font-medium">Poll options — select the correct answer</p>{q.options.map((o,oi)=><div key={o.id} className="flex items-center gap-2"><input type="radio" name={`correct-${q.id}`} checked={o.isCorrect} onChange={()=>updateQuestion(q.id,{options:q.options.map(x=>({...x,isCorrect:x.id===o.id}))})}/><Input value={o.label} onChange={e=>updateOption(q.id,o.id,{label:e.target.value,value:e.target.value})} placeholder={`Option ${oi+1}`}/>{q.options.length>2&&<Button type="button" variant="ghost" size="icon" onClick={()=>updateQuestion(q.id,{options:q.options.filter(x=>x.id!==o.id)})}><Trash2 className="h-4 w-4"/></Button>}</div>)}<Button type="button" size="sm" variant="ghost" onClick={()=>updateQuestion(q.id,{options:[...q.options,{id:crypto.randomUUID(),label:"",value:String.fromCharCode(65+q.options.length),isCorrect:false}]})}><Plus className="mr-1 h-4 w-4"/>Option</Button></div>}{q.type==="drawbox"&&<p className="mt-2 text-xs text-muted-foreground">Learners receive a drawing workspace for this question.</p>}</div>)}{!questions.length&&<div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">Add a question, poll, or drawbox to start.</div>}</div></div>
   </div>
